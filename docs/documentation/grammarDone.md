# README: Unambiguous OSL Grammar

This document explains the unambiguous grammar for OSL (Our Simple Language), as provided in the BNF-style notation. The grammar is designed for a recursive descent parser, ensuring clarity and avoiding ambiguities present in earlier versions (e.g., `grammar.md`). It supports a rich set of features, including typed variables, functions, arrays, pointers, closures, and control flow constructs, tailored for compilation into bytecode for a stack-based virtual machine (VM). This README describes the grammar’s structure, key components, and design choices, with examples to illustrate its syntax.
...
## Comparison with `grammar.md`

The unambiguous grammar improves on `grammar.md` by:
- Removing ambiguous productions (e.g., optional blocks in `ifStmt`).
- Adding explicit types (`Type`, `AtomType`, `CompType`) for clarity.
- Supporting pointers and dereferencing (`Ptr`, `PtrDeref`).
- Enhancing control flow with `elif` and mandatory blocks.
- Simplifying numeric literals (`BIN`, `OCT`, `DEC`, `HEX`) for consistency.
...
log sum(arr); // Prints 6
```

This program demonstrates:
- Array declaration and initialization.
- Function with an array parameter.
- While loop and array access.
- Logging the result.

## Conclusion

The unambiguous OSL grammar provides a clear, parseable specification for a feature-rich language. Its design supports recursive descent parsing, explicit typing, and advanced features like closures, arrays, and pointers. By addressing ambiguities and aligning with the VM’s capabilities (e.g., stack-based execution, linked-list environment), the grammar ensures robust compilation into bytecode, as seen in prior examples. This grammar is ideal for educational purposes, functional programming, and systems programming within the OSL ecosystem.